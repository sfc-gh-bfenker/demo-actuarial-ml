...
def train(
    ...
    try:
        ...
        training_dataset   = load_dataset(session, name="ACTUARIAL_TRAINING",   version=ds_version)
        validation_dataset = load_dataset(session, name="ACTUARIAL_VALIDATION", version=ds_version)
        ...
        df_val["PREDICTED_PURE_PREMIUM"] = gbm.predict(val_sdf).to_pandas()["PREDICTED_PURE_PREMIUM"]
        ...
        fig_lift = double_lift_chart(df_val, {"GBM": df_val["PREDICTED_PURE_PREMIUM"].values}, n_bins=10)
        fig_lift.savefig("/tmp/double_lift.png", dpi=150, bbox_inches="tight")
        plt.close(fig_lift)

        fig_gini, ax = plt.subplots(figsize=(8, 8))
        cum_exp, cum_claims = lorenz_curve(df_val["PURE_PREMIUM"], df_val["PREDICTED_PURE_PREMIUM"], df_val["EXPOSURE"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ds-version",    default="1",             help="Feature Store dataset version")
    parser.add_argument("--model-version", default=default_version, help="Experiment run name / Model Registry version")
